...
public class Cycle{
	...
	public void render(ICanvas canvas, int size){
		canvas.save();
		for (int x = 0; x < this.width; x++){
			for (int y = 0; y < this.height; y++){
				int dx = this.map[x][y];
				if ((dx & 8) == 8){
					canvas.strokeLine((x+0.5)*size, (y+0.5)*size, (x+0.5)*size, y*size, 0xFFFFFFFF, 3);
				}
				if ((dx & 4) == 4){
					canvas.strokeLine((x+0.5)*size, (y+0.5)*size, (x+1)*size, (y+0.5)*size, 0xFFFF0000, 3);
				}
				if ((dx & 2) == 2){
					canvas.strokeLine((x+0.5)*size, (y+0.5)*size, (x+0.5)*size, (y+1)*size, 0xFF00FF00, 3);
				}
				if ((dx & 1) == 1){
					canvas.strokeLine((x+0.5)*size, (y+0.5)*size, x*size, (y+0.5)*size, 0xFF0000FF, 3);
				}

				canvas.fillText(Integer.toString(getIndex(x, y)), (x+0.2)*size, (y+0.5)*size, 0xFFCBD5E1, 0.22*size, TextAlignment.CENTER);
			}
		}
		canvas.restore();
	}
